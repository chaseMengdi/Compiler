...
void yufa_analysis(vector<IDwords> &AnalysisResults)
{
    ...
    if(it->word != "main")
	{
		cout<<"error:缺少main"<<endl;
		exit(-1);
	}
	it = it+3; //跳过“（）”
	if(it->word != "{")
	{
		cout<<"error:main函数缺少'{'"<<endl;
		exit(-1);
	}
	it++;
	//获取变量声明
...
int main(int argc,char* argv[])
{
	...
	string source;
	cout<<"\n在相同目录生成汇编文件（拓展名为.asm）\n"<<endl;
	cout<<"\n请输入源文件的路径:(最好不要含有中文)\n";
	cin>>source; 
	cifa_analysis(source,AnalysisResults);
	yufa_analysis(AnalysisResults);
	print_syntax(); 
